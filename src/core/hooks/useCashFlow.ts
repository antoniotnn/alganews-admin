...
export default function useCashFlow(type: CashFlowEntryType){
    ...
    const updateEntry = useCallback(
        (entryId: number, entry: CashFlow.EntryInput) =>
            dispatch(
                type === 'EXPENSE'
                    ? ExpensesActions.updateExpense({ entryId: entryId, entry: entry })
                    : RevenuesActions.updateRevenue({ entryId: entryId, entry: entry })
            ).unwrap(),
        [dispatch, type]
    );

    const removeEntries = useCallback( (ids: number[]) =>
            dispatch(
                type === 'EXPENSE'
    ...
    const setSelected = useCallback( (keys: Key[]) =>
            dispatch(
                type === 'EXPENSE'
    ...
    const setQuery = useCallback( (query: Partial<CashFlow.Query>) =>
            dispatch(
                type === 'EXPENSE'
